...
//
// developed by Elston Ma for CS134 Project 2
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	bool intersects = false;

	if (node.children.empty()) {
		intersects = node.box.intersect(ray, 0, 99999999.0);
		if (intersects) nodeRtn = node;
	} else {
		for (int i = 0; i < node.children.size(); i++) {
			intersects = intersect(ray, node.children[i], nodeRtn) || intersects;
		}
	}
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	...
	if (node.children.empty()) {
		...
		for (int i = 0; i < node.children.size(); i++) {
			intersects = intersect(box, node.children[i], boxListRtn) || intersects;
		}
	}
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn, vector<TreeNode>& nodeListRtn) {
	...
	if (node.children.empty()) {
		...
		for (int i = 0; i < node.children.size(); i++) {
			intersects = intersect(box, node.children[i], boxListRtn, nodeListRtn) || intersects;
		}
	}
